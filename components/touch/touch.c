...
esp_err_t touch_init(const touch_config_t *config, esp_lcd_touch_handle_t *tp)
{
    ...
    // Skipped when config->bus_already_initialized is set (e.g. touch
    // shares the LCD's SPI bus/pins on this board, and the LCD component
    // already brought that host up before touch_init() runs).
    if (!config->bus_already_initialized) {
