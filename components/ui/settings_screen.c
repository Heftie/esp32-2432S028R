...
void settings_screen_create(void)
{
    ...
    // flex_grow(1) rather than a fixed pixel height — this project's
    // board_config.h selects boards with different resolutions (see
    // main/hardware.h), so a screen's body can't assume a fixed height
    // the way a single-board project's screens could.
